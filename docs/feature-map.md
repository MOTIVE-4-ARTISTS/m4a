...
|---|---|---|---|---|
| `/` | live | static + Supabase + Keystatic | [`app/(marketing)/page.tsx`](../app/(marketing)/page.tsx) | Artist-first (design audit 2026-05): typography-led h1 "the artist comes first", with New York framed as the starting point rather than the boundary. Composes live blocks — three-program application-status strip ([`lib/programs.ts`](../lib/programs.ts)), opportunities preview, cohort spotlight, next-event teaser, and a complete four-program overview that adds Pedagogies — each data-backed block self-suppressing when its source is empty. The cohort spotlight is an auto-rotating [`<ArtistCarousel />`](../components/content/artist-carousel.tsx) of the **whole** most-recent cohort (not a sliced sample); it supplies real artist photography until the cross-disciplinary editorial photo brief is fulfilled. |
...
| `/programs/residency` `/programs/international-exchange` `/programs/discounted-space` `/programs/pedagogies` | live | static | `app/(marketing)/programs/*/page.tsx` | One page per public program. The three intake programs link into `/apply/...` forms; Pedagogies is rolling by inquiry through `/connect`. |
...
| One-shot seed import | live | `pnpm seed:opportunities` | Bootstrap 13-row dataset from the movement community where the resource began; ongoing discovery widens the catalog across disciplines. Embeds rows if Gemini is configured. |
