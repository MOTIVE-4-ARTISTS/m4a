...
|---|---|---|---|
| `/` (home) | donor-first hero; one above-the-fold block; institutional voice | **HIGH** | Full hero rewrite per §6. New blocks: app-status strip, opportunities preview, four-program overview, cohort spotlight. Voice flips to lowercase warm register. |
...
6. **Keep press and partnership credits contextual, not global.** The July review rejected the global press/funder strip: repeating one LLC relationship and one inherited press mention on every page made the site look backward rather than established. Preserve archival coverage on `/press`, name funders on the programs they support, and add a dedicated supporters surface only when the list is substantive. **Impact: MED.**
...
9. **Photography commission (the v2 starter).** Photograph 2–4 artists across disciplines actively making, exchanging feedback, or working with materials. Keep the city and any studio as context, never the subject; avoid an empty room, a generic headshot, or one discipline standing in for the whole invitation. Documentary candid, available light, with one strong horizontal frame and 8–12 supporting images at full resolution. Until these exist, let the typographic hero lead and use the cohort portraits lower on the page. **Effort: 1 shoot day + 1 editing day. Impact: critical for v2.**
