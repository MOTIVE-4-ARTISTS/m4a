...
- **GitHub: account converted to an Organization** — now **github.com/MOTIVE-4-ARTISTS**; repo `m4a` is org-owned. **GitHub for Nonprofits applied 2026-06-30** (~10-day human review → free Team). Note: the local git remote still points to the old `MOtiVE4ARTists` name — update to `MOTIVE-4-ARTISTS` before pushing (GitHub redirects, but cleaner to fix). Still to do before first push: pick the pusher account + `gh auth login` (out-there-dev only had pull access).
